...
class LifeLikePaint extends Component{
    ...
    calculateScore(year2events, year, events, method, selected_person, types){
        ...
        types.forEach(type => {
            type2events[type] = events.filter( event => event.trigger.parent_type===type || event.trigger.type===type || type==='总')
            type2score[type] = 0
        })
    ...
    loadLifeLineData(selected_person){
        ...
        let types = [...triggerManager.types].sort()  //分类
        ...
        years.forEach(year=>{
            ...
            let stack_y = 0
            types.forEach(type=>{
                let this_events = events.filter(event => event.trigger.type===type)
