...
class EventMatrix extends Component {
    ...
    render(){
        ...
        let attr_mark_datas = main_matrix.attr_mark_datas || []
...
function initOneMatrix(selected_person_id, data, start_x, start_y, center_padding, time_len, addr_len, person_len_x, person_len_y, temp_time_range=undefined){
    ...
    for(let event_id in events){
        ...
        attr_mark_datas.push([event_mark_data, color])
    }
    ...
    for(let index in attr_mark_datas){
        let attr_mark_data = attr_mark_datas[index][0]
        let color = attr_mark_datas[index][1]
    ...
    return {
        attr_mark_datas:attr_mark_datas,
