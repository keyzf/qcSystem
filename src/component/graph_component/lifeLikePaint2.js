            ...
            types.forEach(type =>{
                if (type2events[type].length==0) {
                    ...
                    type2score[type] = type2events[type].reduce((total, event)=>{
                        let imp = event.getImp(selected_person)  * Math.exp(-(year-event.time_range[0])/windows_size)
                        let score = event.getScore(selected_person) * imp
    ...
    loadInferMarkData(data, infer, selected_person){
        ...
        for(let year in year2events){
            ...
            let mark_data = events.map((event, index)=>{
                ...
                /* 弧 */
                // return {
                //     angle0: 2*Math.PI/3 + 10/4/Math.PI*score, 
                //     angle: 4*Math.PI/3 + 10/4/Math.PI*score, 
                //     x: yearScale(year),
                //     prob: prob,
                //     y: index * margin_y - 1.0,
                //     events: [event],
                //     opacity: prob, 
                //     radius: 10, 
                //     radius0: 9
